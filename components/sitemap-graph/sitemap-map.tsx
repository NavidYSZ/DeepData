...
function SitemapMapInner({
  ...
  const {
    ...
    stats,
    rootSlug
  } = useMemo(() => transformSitemapToReactFlow(sitemap, { direction: "TB" }), [sitemap]);
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Root info footer (small) */}
      {rootSlug ? (
        <div className="text-[11px] text-muted-foreground">
          Root: <code className="font-mono">{rootSlug}</code> · max Tiefe: {stats.maxDepth}
        </div>
      ) : null}
    </div>
  );
